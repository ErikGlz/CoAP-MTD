...
def dexor_payload(xored_payload):
    # Función para deshacer XOR en el payload
    mask_value = len(xored_payload) % 256
    return bytes(b ^ mask_value for b in xored_payload)
...
async def fetch_resource(protocol, uri, payload=None):
    ...
    try:
        ...
        if dialecto_predicho != 0 and dialecto_predicho == global_config.selected_dialect_index+1:
            ...
            # Deshacer XOR en el Payload recibido
            xored_payload = response.payload
            dexored_payload = dexor_payload(xored_payload)
            ...
            #print(f"Payload: {xored_payload.hex()}")  # En hexadecimal
            print(f"Payload: {dexored_payload.decode('utf-8')}")
            print(colored("--------------------------------------------------------------------------","green"))
