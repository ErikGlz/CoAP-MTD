...
# Cantidad de posiciones a rotar los bits (debe coincidir con el servidor)
ROTATION_BITS = 3

def rotate_right(value, n):
    """Función auxiliar para rotar bits hacia la derecha"""
...
async def fetch_resource(protocol, uri, payload=None):
    ...
    try:
        ...
        if dialecto_predicho != 0 and dialecto_predicho == global_config.selected_dialect_index+1:
            ...
            # Desrotar bits del Payload recibido
            rotated_payload = response.payload
            derotated_payload = bytes(rotate_right(byte, ROTATION_BITS) for byte in rotated_payload)
            ...
            #print(f"Payload (rotado): {rotated_payload.hex()}")
            print(f"Payload: {derotated_payload.decode('utf-8')}")
            print(colored("--------------------------------------------------------------------------","green"))
